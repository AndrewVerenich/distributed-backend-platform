...
### JWKS (JSON Web Key Set) — для production

Auth Service может предоставлять endpoint:
```
GET /auth/.well-known/jwks.json
```

Gateway и другие сервисы периодически обновляют public keys оттуда. Это позволяет:
